...
def test_statistics_simple2():
    ...
    sim = Simulation('stations.json', 'sample_rides.csv')
    pygame.event.post(
        pygame.event.Event(pygame.QUIT, {}))  # Ignore this line

    sim.run(datetime(2017, 6, 1, 9, 32, 0),
            datetime(2017, 6, 1, 9, 32, 0))
    stats = sim.calculate_statistics()
